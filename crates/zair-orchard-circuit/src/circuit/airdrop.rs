    ...
    fn synthesize(
        ...
        if scheme == ValueCommitmentScheme::Sha256 {
            ...
            let half = |lo: usize, hi: usize| {
                value_le_bytes.map(|bs| bs[lo] * pallas::Base::from(1u64 << 8) + bs[hi])
            };
            ...
            let (b_cells, w1_hi_c, w1_lo_c, w2_hi_c, w2_lo_c) = layouter.assign_region(
                ...
                |mut region| {
                    ...
                    let w1_hi_c =
                        region.assign_advice(|| "W_1_hi", config.advices[0], 1, || w1_hi_v)?;
                    let w1_lo_c =
                        region.assign_advice(|| "W_1_lo", config.advices[1], 1, || w1_lo_v)?;
                    let w2_hi_c =
                        region.assign_advice(|| "W_2_hi", config.advices[2], 1, || w2_hi_v)?;
                    let w2_lo_c =
                        region.assign_advice(|| "W_2_lo", config.advices[3], 1, || w2_lo_v)?;
